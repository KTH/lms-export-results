...
const canvasApi = canvas(canvasApiUrl, process.env.CANVAS_TOKEN)
...
test('should construct the similar json', async t => {
  const sections = []
  for await (const section of canvasApi.list(`courses/3719/sections`, { 'include[]': 'students' })) {
    sections.push(section)
  }

  // Get the json, sorted, without the test user. The new function don't incluce the test users.
  const oldWay = []
  for await (const submission of canvasApi.list(`courses/3719/students/submissions`, { grouped: 1, 'student_ids[]': 'all' })) {
    oldWay.push(submission)
  }
  oldWay.sort((a, b) => a.user_id - b.user_id).filter(stud => stud.user_id !== 55842)
  ...
  assert.deepStrictEqual(oldWay, newWay)
