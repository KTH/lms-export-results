  ...
  return sections.reduce(
    (
      ...
      const studentsInSection = currentSection.students.reduce(
        (accumulatedStudents, currentStudent, currentIndex, studentsSource) => {
          if (
...
async function getSubmissions ({ canvasCourseId, sections, canvasApi, log }) {
  const studentsFromSections = extractUniqueStudentsAndSectionNamesFromSections(
    sections
  )
